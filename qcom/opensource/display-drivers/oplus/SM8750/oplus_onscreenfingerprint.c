...
static void ofp_owned_uiready_work_handler(struct work_struct *work_item);
...
{
	if (!params || !READ_ONCE(params->uiready_compat))
		return NULL;
...
static struct ofp_uiready_owner *ofp_owner_for_panel(struct dsi_panel *panel)
{
	...
	if (!panel)
		return NULL;
	/* A partial dsi_panel_get() failure may precede OFP construction. */
	for (i = 0; i < ARRAY_SIZE(g_ofp_uiready_owners); i++) {
		if (READ_ONCE(g_ofp_uiready_owners[i].panel) == panel)
			return &g_ofp_uiready_owners[i];
	}
	return NULL;
}

/* All request transitions and their sequence numbers are under state_lock. */
static void ofp_request_terminal(struct ofp_uiready_request *request)
{
		...
		struct ofp_uiready_request *request)
{
	...
	if (oplus_ofp_log_now_level >= OPLUS_OFP_LOG_LEVEL_INFO)
		++owner->transition_seq;
	ofp_request_terminal(request);
}
		...
		unsigned int value, bool level_change, bool rearm)
{
	...
	queue_work(owner->uiready_wq, &owner->uiready_work);
}
...
void oplus_ofp_stop(struct dsi_panel *panel)
{
	...
	if (oplus_ofp_log_now_level >= OPLUS_OFP_LOG_LEVEL_INFO)
		++owner->transition_seq;
	ofp_request_cancel(owner, &owner->pending_ready);
	ofp_request_cancel(owner, &owner->pending_off);
...
int oplus_ofp_init(void *dsi_panel)
{
	int rc = 0;
	int init_rc = 0;
	struct ofp_uiready_owner *owner = NULL;
	unsigned long flags;
	...
	/* Do not relatch the path or reinitialize locks for a bound session. */
	...
	if (p_oplus_ofp_params->uiready_compat) {
		...
		spin_unlock_irqrestore(&owner->state_lock, flags);
		INIT_WORK(&owner->uiready_work, ofp_owned_uiready_work_handler);
	}
	...
	if (oplus_ofp_is_supported()) {
		...
		if (!oplus_ofp_oled_capacitive_is_enabled()) {
			...
			/* Keep the original notifier publication point for legacy panels. */
			if (!owner) {
	...
	/* Optional parsing remains best effort; only construction errors are fatal. */
	return init_rc;
}
...
static int ofp_send_owned_uiready_event(struct ofp_uiready_owner *owner, unsigned int ui_status)
{
	enum panel_event_notification_type notify_type = DRM_PANEL_EVENT_ONSCREENFINGERPRINT_UI_DISAPPEAR;
...
static void ofp_owned_uiready_work_handler(struct work_struct *work_item)
{
	...
	for (;;) {
		...
		ofp_send_owned_uiready_event(owner, request.value);

		spin_lock_irqsave(&owner->state_lock, flags);
		ofp_request_terminal(&owner->in_flight);
		spin_unlock_irqrestore(&owner->state_lock, flags);
	}
}

/* notify uiready */
static int ofp_notify_uiready_legacy(void *sde_encoder_phys)
{
	uint64_t hbm_enable = 0;
...
/* Called only for the opted-in owner, with state_lock held. Readiness and
 * READY-only rearm consumption are the same as the existing notify path.
 */
...
int oplus_ofp_notify_uiready(void *sde_encoder_phys)
{
	...
	owner = ofp_owner_for_params(p_oplus_ofp_params);
	if (!owner)
		return ofp_notify_uiready_legacy(sde_encoder_phys);
...
static int oplus_ofp_aod_off_set(void)
{
	...
	if (!owner && (IS_ERR_OR_NULL(p_oplus_ofp_params->aod_off_set_wq)
...
int oplus_ofp_touchpanel_event_notifier_call(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	if (tp_event) {
		if (action == EVENT_ACTION_FOR_FINGPRINT) {
			...
			if (owner && oplus_ofp_local_hbm_is_enabled()
					&& oplus_ofp_local_hbm_unlocking_acceleration_is_enabled())
			{
				spin_lock_irqsave(&owner->state_lock, flags);
				if (owner->lifecycle != OFP_OWNER_ACCEPTING) {
